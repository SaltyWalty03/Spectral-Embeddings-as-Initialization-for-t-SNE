# Author: Walter Brookby <w.p.bropokby@gmail.com>
#
...
from UMAP.spectral_custom import spectral_layout, tswspectral_layout
...
# Added for spectral init
def pij_to_sparse(P: np.ndarray, topk: int = 50) -> sp.csr_matrix:
    """Convert dense p_ij (symmetric or not) to a symmetric top-k sparse graph."""
    P = 0.5 * (P + P.T)
...
def spectral_layout_from_pij(P: np.ndarray, n_components: int) -> np.ndarray:
    """LE init on p_ij graph: (D - W) v = λ D v, drop the trivial eigenvector."""
    W = pij_to_sparse(P, topk=50)  # tune topk if needed
    d = np.asarray(W.sum(axis=1)).ravel()
    d_safe = np.where(d > 0, d, 1e-12)
    D = sp.diags(d_safe, format="csr")
    L = D - W

    k = min(n_components + 1, W.shape[0] - 1)
    # shift-invert near 0 to get smallest eigenpairs of the generalized problem
    vals, vecs = eigsh(L, M=D, k=k, sigma=0.0, which="LM")
    return vecs[:, 1:n_components + 1].astype(np.float32)

# Normal UMAP all below
...
def compute_pairwise_distances(X):
    """
    Computes the pairwise Euclidean distance matrix D for input X.
    """
    # ||x_i - x_j||^2 = ||x_i||^2 + ||x_j||^2 - 2 x_i^T x_j
    squared_norms = np.sum(X**2, axis=1)  # shape: (n, 1)
    D_squared = squared_norms + squared_norms.T - 2 * np.dot(X, X.T)
    D_squared = np.maximum(D_squared, 0)  # numerical stability
    D = np.sqrt(D_squared)
    return D

def compute_pairwise_affinities(X):
    """
    Returns p_ij matrix using: p_ij = 1 / (1 + ||x_i - x_j||^2)
    """
    D = compute_pairwise_distances(X)
    P = 1 / (1 + D ** 2)
    np.fill_diagonal(P, 0)   # set p_ii = 0
...
def simplicial_set_embedding(
    ...
    elif isinstance(init, str) and init == 'prof_spectral':
        P = compute_pairwise_affinities(data)
        embedding = spectral_layout_from_pij(P, n_components)
        embedding = noisy_scale_coords(embedding, random_state, max_coord=10, noise=1e-4)
    else:
...
class UMAP(BaseEstimator, ClassNamePrefixFeaturesOutMixin):
    ...
    def _validate_parameters(self):
        ...
        if isinstance(self.init, str) and self.init not in (
            ...
            raise ValueError(
                ...
                ' "spectral", "random", or "prof_spectral"'
            )
        if (
    ...
    def fit(self, X, y=None, **kwargs):
        ...
        **kwargs : optional
        ...
        self.p_ij = compute_pairwise_affinities(X)
        self.p_ij = csr_matrix(self.p_ij)
        self.graph_ = self.p_ij
        #self.P_ = compute_pairwise_affinities(X)
...
def spectral_from_W(W: sp.csr_matrix, n_components: int,
                    sigma_shift: float = 1e-3, extras: int = 8, tol: float = 1e-3):
    """
    Laplacian Eigenmaps on sparse adjacency W:
        (D - W) v = λ D v, drop all near-zero eigenvectors.
    Uses a tiny positive shift for stable shift-invert, with a safe fallback.
    """
    # Symmetrize & clean (cheap insurance)
    W = 0.5 * (W + W.T)
    W.setdiag(0.0); W.eliminate_zeros()
    if W.nnz == 0:
        raise ValueError("Empty graph W; increase n_neighbors/topk.")
    if not np.isfinite(W.data).all():
        raise ValueError("Non-finite weights in W.")

    # Degrees with a floor so D is positive definite
    d = np.asarray(W.sum(axis=1)).ravel()
    d_safe = np.where(d > 0, d, 1e-12)
    D = sp.diags(d_safe, format="csr")
    L = (D - W).astype(np.float64)

    n = W.shape[0]
    k = min(n_components + extras, n - 1)
    ncv = min(2 * k + 1, max(2*k+1, 20), n - 1)  # a little oversampling helps

    try:
        # FAST & STABLE: shift-invert around a tiny positive sigma so (L - σD) is invertible
        vals, vecs = eigsh(L, M=D, k=k,
                           sigma=sigma_shift, which="LM",
                           tol=tol, ncv=ncv, maxiter=None)
    except Exception:
        # Fallback: no shift, ask for the smallest eigenpairs directly (slower but safe)
        vals, vecs = eigsh(L, M=D, k=k,
                           which="SM", tol=tol, ncv=ncv, maxiter=None)

    # Sort, drop near-zero eigenvalues (one per connected component), keep next n_components
    order = np.argsort(vals)
    vals, vecs = vals[order], vecs[:, order]
    drop = int((vals < 1e-8).sum())
    Y = vecs[:, drop:drop + n_components]
    if Y.shape[1] != n_components:
        # Last resort: if the graph is extremely fragmented, just take the first n_components
        Y = vecs[:, 1:1 + n_components]
