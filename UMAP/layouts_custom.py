...
from UMAP.umap_scratch import fuzzy_simplicial_set
...
def _optimize_layout_euclidean_single_epoch(
    ...
    for i in numba.prange(epochs_per_sample.shape[0]):
        if epoch_of_next_sample[i] <= n:
            ...
            if densmap_flag:
                # testing new similarity functions
...
def optimize_layout_euclidean(
    ...
    try:
        ...
        graph, _, _ = fuzzy_simplicial_set(X, n_neighbors=15, metric='euclidean', random_state=42)
        p_ij_array = graph.data.astype(np.float32)
