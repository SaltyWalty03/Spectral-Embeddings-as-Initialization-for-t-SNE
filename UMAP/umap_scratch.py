...
def umap_cost_debug(Y, P, a=1.0, b=1.0, neg_sample_rate=5, rng=None, eps=1e-12):
    """
    Approximate UMAP cross-entropy at embedding Y.

    Parameters
    ----------
    Y : (n, d) float array        current embedding
    P : (n, n) sparse CSR/COO     symmetric high-D affinities (p_ij), use the graph you train with
    a, b : float                  low-D kernel params (UMAP uses a,b from (min_dist, spread); often a=b=1)
    neg_sample_rate : int         number of negatives per positive (match your training)
    rng : int or Generator        for reproducible sampling
    eps : float                   numerical stability

    Returns
    -------
    C : float  (approximate cost)
    """
    if not sp.isspmatrix(P):
        raise TypeError("P must be a scipy sparse matrix.")

    # --- Attractive term: -sum_{i<j} p_ij log q_ij over edges
    P_ut = sp.triu(P, k=1).tocoo()
    i, j = P_ut.row, P_ut.col
    pij = P_ut.data

    d2 = ((Y[i] - Y[j])**2).sum(axis=1)
    u = a * (d2**b)
    q = 1.0 / (1.0 + u)
    attractive = -np.sum(pij * np.log(q + eps))

    # --- Repulsive term: negative sampling (no p_ij weight, as in UMAP optimization)
    m = pij.size
    n_neg = max(1, int(neg_sample_rate * m))
    rng = np.random.default_rng(rng)

    ii = rng.integers(0, Y.shape[0], size=n_neg)
    jj = rng.integers(0, Y.shape[0], size=n_neg)
    mask = ii != jj
    ii, jj = ii[mask], jj[mask]

    d2n = ((Y[ii] - Y[jj])**2).sum(axis=1)
    un = a * (d2n**b)
    qn = 1.0 / (1.0 + un)
    repulsive = -np.sum(np.log(1.0 - qn + eps))

    return float(attractive + repulsive)
...
class UMAP_scratch:
    ...
    def _initialize_embedding(self, X, graph):
        if self.init == "random":
            ...
            deg = np.asarray(Wp.sum(axis=1)).ravel()
            keep = deg > 1e-10
            dropped = int((~keep).sum())
            if dropped and self.verbose:
                print(f"[init] dropping {dropped} isolated points from spectral init")
    ...
    def fit(self, X):
        ...
        if self.verbose:
            # pass an integer seed because umap_cost_debug uses default_rng
            seed = int(self.random_state.randint(0, 2**31 - 1))
            print("[init] C≈",
                f"{umap_cost_debug(self.embedding_, self.graph_, a=self._a, b=self._b, rng=seed):.4f}")
