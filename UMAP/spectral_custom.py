...
def _spectral_layout(
    ...
    # 1) Build sparse degree‐matrix and Laplacian: L = D – P
    deg = np.array(graph.sum(axis=1)).squeeze()
    D = sp.diags(deg)       # sparse diagonal
    L = D - graph           # still sparse

    # 2) Compute the smallest (dim+1) eigenpairs of L
    ...
    # 3) Discard the first (constant) mode, keep next `dim`:
    V = eigvecs[:, 1 : dim+1]   # shape (n_samples, dim)

    # 4) Scale by sqrt((n-1)*δ/(2*dim)) exactly as in your Eq.(3.1)
    n = graph.shape[0]
    scale = np.sqrt((n - 1) * delta / (2 * dim))
    Y = V * scale
